...
import Fly from "../commonjs/Fly.js";
...
describe("Fly", () => {
	...
	describe("construction", () => {
		...
		it("throws an exception if the provided username is not a string", () => {
			expect(() => {
				...
				Error,
				'Invalid parameter(s): ["username" must be a string].',
			);
		});
		...
		it("throws an exception if the provided password is not a string", () => {
			expect(() => {
				...
				Error,
				'Invalid parameter(s): ["password" must be a string].',
			);
		});
	});
	...
	describe("login", () => {
		it("throws an exception if the username is not provided", async () => {
			try {
				await newConcourse().login({ password: "some-password" });
			} catch (e) {
				expect(e instanceof Error).to.eql(true);
				expect(e.message).to.eql(
					'Invalid parameter(s): ["username" is required].',
				);
				return;
			}
		...
		it("throws an exception if the provided username is not a string", async () => {
			try {
				await newConcourse().login({ username: 25, password: "some-password" });
			} catch (e) {
				expect(e instanceof Error).to.eql(true);
				expect(e.message).to.eql(
					'Invalid parameter(s): ["username" must be a string].',
		...
		it("throws an exception if the password is not provided", async () => {
			try {
				await newConcourse().login({ username: "some-username" });
			} catch (e) {
				expect(e instanceof Error).to.eql(true);
				expect(e.message).to.eql(
					'Invalid parameter(s): ["password" is required].',
				);
				return;
			}
			expect.fail(null, null, "Expected exception but none was thrown.");
		...
		it("throws an exception if the provided password is not a string", async () => {
			try {
				await newConcourse().login({ username: "some-username", password: 25 });
			} catch (e) {
				expect(e instanceof Error).to.eql(true);
				expect(e.message).to.eql(
					'Invalid parameter(s): ["password" must be a string].',
		...
		it("uses the provided team name instead of that provided at construction", async () => {
			const fly = await newConcourse({ teamName: "initial" }).login({
				username: "some-username",
				password: "some-password",
				teamName: "overridden",
			});
	...
	describe("jobs", () => {
		it("throws an exception if no pipeline is provided", async () => {
			try {
				...
				expect(e instanceof Error).to.eql(true);
				expect(e.message).to.eql(
					'Invalid parameter(s): ["pipeline" is required].',
				);
				return;
			}
			expect.fail(null, null, "Expected exception but none was thrown.");
		...
		it("throws an exception if the provided pipeline is not a string", async () => {
			try {
				...
				expect(e instanceof Error).to.eql(true);
				expect(e.message).to.eql(
					'Invalid parameter(s): ["pipeline" must be a string].',
				);
				return;
			}
			expect.fail(null, null, "Expected exception but none was thrown.");
		...
		it("gets jobs for the supplied team and pipeline", async () => {
			...
			const authToken = build.api.tokenResponseBodyPreVersion4({
				value: bearerToken,
			});
			...
			mock
				.onGet(`${uri}/teams/${teamName}/auth/token`, {
					headers: {
			...
			mock
				.onGet(`${uri}/teams/${teamName}/pipelines/${pipelineName}/jobs`, {
					headers: {
			...
			const fly = await new Fly({ uri, teamName }).login({
				username,
				password,
			});
	...
	describe("pipelines", () => {
		it("throws an exception if the value provided for all is not a boolean", async () => {
			try {
				...
				expect(e instanceof Error).to.eql(true);
				expect(e.message).to.eql(
					'Invalid parameter(s): ["all" must be a boolean].',
				);
				return;
			}
			expect.fail(null, null, "Expected exception but none was thrown.");
		...
		it("gets pipelines for the supplied team by default", async () => {
			...
			mock
				.onGet(`${uri}/teams/${teamName}/pipelines`, {
					headers: {
						Authorization: `Bearer ${bearerToken}`,
					},
				})
	...
	describe("builds", () => {
		it("throws an exception if the value provided for count is not a number", async () => {
			try {
				await newConcourse().builds({ count: "badger" });
			} catch (e) {
				expect(e instanceof Error).to.eql(true);
				expect(e.message).to.eql(
					'Invalid parameter(s): ["count" must be a number].',
				);
				return;
			}
			expect.fail(null, null, "Expected exception but none was thrown.");
		...
		it("throws an exception if the value provided for count is not an integer", async () => {
			try {
				await newConcourse().builds({ count: 32.654 });
			} catch (e) {
				expect(e instanceof Error).to.eql(true);
				expect(e.message).to.eql(
					'Invalid parameter(s): ["count" must be an integer].',
				);
				return;
			}
			expect.fail(null, null, "Expected exception but none was thrown.");
		...
		it("throws an exception if the value provided for count is less than 1", async () => {
			try {
		...
		it("throws an exception if the value provided for team is not a boolean", async () => {
			try {
		...
		it("throws an exception if the provided job is not a string", async () => {
			try {
				await newConcourse().builds({ job: 30 });
			} catch (e) {
				expect(e instanceof Error).to.eql(true);
				expect(e.message).to.eql(
					"Invalid parameter(s): [" + '"job" must be a string].',
				);
				return;
			}
			expect.fail(null, null, "Expected exception but none was thrown.");
		...
		it("throws an exception if the provided job is not in the format " +
			"pipeline/job", async () => {
			try {
		...
		it("throws an exception if both pipeline and job are provided", async () => {
			try {
		...
		it("gets all builds with a limit of 50 by default", async () => {
			const uri = "https://concourse.example.com";
		...
		it("gets all builds limited by count when specified", async () => {
			const uri = "https://concourse.example.com";
		...
		it("passes no limit when count is null", async () => {
			const uri = "https://concourse.example.com";
		...
		it("gets builds for job when provided", async () => {
			const uri = "https://concourse.example.com";
		...
		it("gets builds for pipeline when provided", async () => {
			const uri = "https://concourse.example.com";
			...
			mock
				.onGet(`${uri}/teams/${teamName}/pipelines/${pipelineName}/builds`, {
					headers: {
						Authorization: `Bearer ${bearerToken}`,
					},
					params: {
		...
		it("gets builds for team when team is true", async () => {
			const uri = "https://concourse.example.com";
			...
			mock
				.onGet(`${uri}/teams/${teamName}/builds`, {
					headers: {
						Authorization: `Bearer ${bearerToken}`,
					},
					params: {
		...
		it("gets all builds when team is false", async () => {
			const uri = "https://concourse.example.com";
