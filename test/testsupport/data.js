...
import { toUnixTime } from "../../commonjs/support/date.js";

// Generate the key pair once at the module level
const key = new NodeRSA({ b: 2048 });
const rsaPrivateKey = key.exportKey("pkcs8-private-pem");
// We might need the public key later if we were verifying, but for signing, private is enough
// const rsaPublicKey = key.exportKey("pkcs8-public-pem");
...
const randomPassword = () => faker.string.alphanumeric(40);
const randomEmail = () => faker.internet.email();

const randomDigit = () => faker.number.int({ max: 9 });
const randomThreeDigits = () =>
	`${randomDigit()}${randomDigit()}${randomDigit()}`;

const randomInfo = (overrides = {}) => ({
	version: randomThreeDigits(),
	workerVersion: randomThreeDigits(),
...
const randomSubject = () => faker.string.alphanumeric(30);
const randomAccessTokenHash = () => faker.string.alphanumeric(22);
...
const randomBearerTokenCurrent = () => faker.string.alphanumeric(38);
...
const randomBuildId = () => faker.number.int();
const randomBuildName = () => `${randomDigit()}.${randomDigit()}`;
const buildStatuses = [
	"pending",
	"started",
	"succeeded",
...
const randomBuildStatus = () => faker.helpers.arrayElement(buildStatuses);
...
const randomContainerType = () =>
	faker.helpers.arrayElement(["check", "put", "get"]);
...
const randomTeamId = () => faker.number.int();
const randomTeamName = () => faker.internet.domainWord();

const randomPipelineId = () => faker.number.int();
const randomPipelineName = () => faker.internet.domainWord();
...
const randomResourceTypeType = () =>
	faker.helpers.arrayElement(["git", "docker-image", "slack-notification"]);
...
const randomPlatform = () => faker.helpers.arrayElement(["linux", "darwin"]);

const randomWorkerAddress = () => faker.internet.ip();
const randomWorkerBaggageclaimUrl = () => `http://${faker.internet.ip()}:45995`;
const randomWorkerActiveContainersCount = () => faker.number.int();
const randomWorkerActiveVolumesCount = () => faker.number.int();
const randomWorkerTags = () => null;
const randomWorkerName = () => faker.internet.domainWord();
const randomWorkerStartTime = () => toUnixTime(faker.date.past());
const randomWorkerState = () =>
	faker.helpers.arrayElement(["running", "stalled"]);
const randomWorkerVersion = () => "2.1";

const randomWorkerResourceTypeImage = () =>
	`/concourse-work-dir/3.14.1/assets/resource-images/${randomLowerCaseWord()}/rootfs`;
...
const randomTeamAuthenticationUser = () =>
...
const randomTeamAuthenticationGroup = () =>
	faker.helpers.arrayElement(["github:some-org-1", "github:some-org-2"]);
...
const randomResourceVersionId = () => faker.number.int();
...
const randomJobId = () => faker.number.int();
const randomJobName = () => faker.internet.domainWord();
...
const randomVolumeId = () =>
	`${randomLowerHex(8)}-${randomLowerHex(4)}-${randomLowerHex(4)}-` +
	`${randomLowerHex(4)}-${randomLowerHex(12)}`;
const randomVolumePath = () =>
	faker.helpers.arrayElement([`/tmp/${randomResourceName()}/get`, "/scratch"]);
