...
import Input from "../../commonjs/model/Input.js";
	...
	describe("requiresAtLeastJobToHavePassed", () => {
		it("returns true if the input requires the job with the supplied name to " +
			...
			const input = new Input(
				data.randomInput({ passed: [job1Name, job2Name] }),
			);

			expect(input.requiresJobToHavePassed(job1Name)).to.eql(true);
		});
		...
		it("returns false if the input does not require the job with the supplied " +
			...
			expect(input.requiresJobToHavePassed(otherJobName)).to.eql(false);
		});
	});
