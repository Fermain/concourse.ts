...
import Pipeline from "../../commonjs/model/Pipeline.js";
...
describe("Pipeline", () => {
	...
	it("knows all start point jobs in the pipeline", async () => {
		...
		expect(actualStartPointJobs).to.be.an("array").with.lengthOf(1);
		expect(actualStartPointJobs[0].getId()).to.equal(
			expectedStartPointJob.getId(),
		);
	});
	...
	it("knows all end point jobs in the pipeline", async () => {
		...
		expect(actualEndPointJobs).to.be.an("array").with.lengthOf(1);
		expect(actualEndPointJobs[0].getId()).to.equal(expectedEndPointJob.getId());
	});
	...
	it("knows all mid point jobs in the pipeline", async () => {
		...
		expect(actualMidPointJobs).to.be.an("array").with.lengthOf(1);
		expect(actualMidPointJobs[0].getId()).to.equal(expectedMidPointJob.getId());
	});
	...
	it("knows all automatic jobs in the pipeline", async () => {
		...
		const expectedAutomaticJobIds = [
			firstAutomaticJob.getId(),
			secondAutomaticJob.getId(),
		];

		const actualAutomaticJobIds = actualAutomaticJobs.map((j) => j.getId());
		expect(actualAutomaticJobIds).to.have.members(expectedAutomaticJobIds);
		expect(actualAutomaticJobIds).to.have.lengthOf(
			expectedAutomaticJobIds.length,
		);
	});
	...
	it("knows all manual jobs in the pipeline", async () => {
		...
		const expectedManualJobIds = [
			secondManualJob.getId(),
			thirdManualJob.getId(),
		];

		const actualManualJobIds = actualManualJobs.map((j) => j.getId());
		expect(actualManualJobIds).to.have.members(expectedManualJobIds);
		expect(actualManualJobIds).to.have.lengthOf(expectedManualJobIds.length);
	});
});
