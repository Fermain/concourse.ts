...
import { createSessionInterceptor, SessionCredentials } from './session.js' // Assuming session.ts exists
...
export const createHttpClient = (
  { credentials, timeout = 5000 }: HttpClientOptions
): HttpClient => { // Return HttpClient type alias
  ...
  if (credentials) {
    ...
    const sessionInterceptor = createSessionInterceptor({ credentials, httpClient: instance });
