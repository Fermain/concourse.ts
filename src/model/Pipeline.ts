...
import Job, { toJob } from './Job.js'
import JobSet from './JobSet.js'
import { Pipeline as PipelineData } from '../types/pipeline.js'
...
interface PipelineConstructorParams extends Omit<PipelineData, 'team_name'> {
  teamName: string;
  client: ConcourseClient;
  // Explicitly include potential extra data if needed
...
export default class Pipeline {
  ...
  static async load (params: PipelineLoadParams): Promise<Pipeline> {
    ...
    // Assumes client method returns data matching PipelineData interface
    ...
    const constructorParams: PipelineConstructorParams = {
      ...pipelineData,
      teamName: pipelineData.team_name,
      client
    }
  ...
  constructor (params: PipelineConstructorParams) {
    ...
    const { id, name, teamName, paused, public: isPublic, api_url, url, client, ...rest } = params;
    this.extraData = rest;
  }
